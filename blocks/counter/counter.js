...
const config = {
  animationDuration: 2000, // Total animation duration in ms
...
function createDigitElement(digit) {
  ...
  // Create number stack for smooth animation
...
async function animateDigit(digitElement, targetValue, delay = 0) {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      const totalDuration = config.animationDuration / config.frameRate;
      let currentValue = 0;
...
async function animateCounter(block) {
  ...
  const animationPromises = digitElements.map((digitElement, index) => {
    ...
    const delay = index * 100; // Stagger animation by 100ms per digit
...
function processBlockContent(block) {
  ...
  if (numberDiv) {
    ...
    if (numberText && !isNaN(parseInt(numberText, 10))) {
